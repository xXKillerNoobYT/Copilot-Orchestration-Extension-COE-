...
describe('LaravelServerController', () => {
    ...
    describe('Server Startup', () => {
        it('should start Laravel server successfully', async () => {
            ...
            const statusPromise = controller.startServer('/workspace');
        ...
        it('should handle health check timeout', async () => {
            ...
            // Health check never responds - timeout on the request
            const mockHttpRequest = new EventEmitter();
            let timeoutFn: Function | undefined;
            (mockHttpRequest as any).setTimeout = jest.fn((ms: number, fn: Function) => {
                timeoutFn = fn;
            });
            (mockHttpRequest as any).destroy = jest.fn();
            ...
            // Advance timers for retries and eventual timeout
    ...
    describe('Health Check', () => {
        it('should return true for healthy server', async () => {
            ...
            (http.get as jest.Mock).mockImplementation((url, callback) => {
                setTimeout(() => callback(mockHttpResponse), 10);
                return mockHttpRequest;
            });
        ...
        it('should return false on connection error', async () => {
            ...
            (http.get as jest.Mock).mockImplementation(() => {
                setTimeout(() => mockHttpRequest.emit('error', new Error('Connection refused')), 10);
                return mockHttpRequest;
            });
