...
export class AgentProfileLoader {
  ...
  validateProfile(profile: any): ValidationResult {
    ...
    if (!profile.version || !/^\d+\.\d+\.\d+(-[a-zA-Z0-9.-]+)?$/.test(profile.version)) {
      errors.push({
        field: 'version',
        message: 'Version must be in semver format (e.g., 1.0.0 or 1.0.0-alpha)',
    ...
    if (profile.permissions) {
      const booleanPermissions = ['read', 'write', 'execute', 'test', 'approve'];
      for (const perm of booleanPermissions) {
        if (profile.permissions[perm] !== undefined && typeof profile.permissions[perm] !== 'boolean') {
          errors.push({
            field: `permissions.${perm}`,
            message: `Permission '${perm}' must be a boolean`,
            value: profile.permissions[perm],
          });
        }
      }

      if (profile.permissions.apiAccess !== undefined && !Array.isArray(profile.permissions.apiAccess)) {
        errors.push({
          field: 'permissions.apiAccess',
          message: 'apiAccess must be an array of strings',
          value: profile.permissions.apiAccess,
        });
      }

      if (profile.permissions.filePatterns !== undefined && !Array.isArray(profile.permissions.filePatterns)) {
        errors.push({
          field: 'permissions.filePatterns',
          message: 'filePatterns must be an array of strings',
          value: profile.permissions.filePatterns,
        });
      }
    }
