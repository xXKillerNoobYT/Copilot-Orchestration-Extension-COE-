...
export class TaskManager {
    ...
    `);
        stmt.run(id, input.project_id, input.parent_task_id || null, input.github_issue_id || null, input.github_issue_url || null, input.name, input.description || null, input.task_type, input.priority || 'medium', input.status || 'pending', input.assigned_agent || null, input.assigned_github_agent || null, input.branch_name || null, input.context_bundle_path || null, input.estimated_effort || null, input.actual_effort || null, now, now);
     ...
     */
    updateTaskStatus(taskId, status, details = {}, expectedVersion) {
        ...
        if (task.version !== expectedVersion) {
            throw new Error(`Concurrent modification detected. Expected version ${expectedVersion}, found ${task.version}`);
        }
        const now = new Date().toISOString();
     ...
     * Add task dependency
     */
    addDependency(taskId, dependsOnTaskId, type = 'blocks') {
        const id = randomUUID();
        const now = new Date().toISOString();
        const stmt = this.db.prepare(`
    ...
    logAuditEntry(entry) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(id, entry.task_id, entry.action, entry.agent_type || null, entry.user_id || null, entry.details || null, now);
        return {
            id,
            ...entry,
            timestamp: now,
     ...
     */
    getAuditLog(taskId) {
        const stmt = this.db.prepare('SELECT * FROM audit_log WHERE task_id = ? ORDER BY timestamp DESC');
        return stmt.all(taskId);
    }
    /**
