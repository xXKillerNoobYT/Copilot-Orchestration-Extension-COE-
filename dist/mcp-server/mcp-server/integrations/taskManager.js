...
export class TaskManager {
    ...
    async getNextTask(options) {
        ...
        let availableTasks = tasks.filter((task) => task.status === 'pending' || task.status === 'in-progress');
        ...
        if (options?.filter) {
            ...
            availableTasks = availableTasks.filter((task) => task.title.toLowerCase().includes(filterLower) ||
     ...
     */
    async updateTaskStatus(taskId, status, progress, observations, blockers) {
        try {
            const tasks = await this.loadTasks();
    ...
    enrichTaskWithContext(task) {
        ...
        if (task.description.toLowerCase().includes('node'))
