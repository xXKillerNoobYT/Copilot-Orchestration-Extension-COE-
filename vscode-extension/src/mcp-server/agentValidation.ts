...
/**
 * Agent-compatible error response format
 */
...
export function validateInput<T>(
  ...
  input: any
): { valid: true; data: T } | { valid: false; error: AgentError } {
  try {
 ...
 */
export function formatAgentError(error: AgentError) {
  return {
    content: [
      {
        type: 'text',
 ...
 */
export function formatAgentSuccess(data: any) {
  return {
    content: [
      {
        type: 'text',
...
export const AgentErrors = {
  taskNotFound: (taskId: string): AgentError => ({
  ...
  operationFailed: (operation: string, reason: string): AgentError => ({
  ...
  internalError: (message: string): AgentError => ({
    code: AgentErrorCode.INTERNAL_ERROR,
